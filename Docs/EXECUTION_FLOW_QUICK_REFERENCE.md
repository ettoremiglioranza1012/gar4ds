...
## ⚙️ Temporal Aggregation Options

The pipeline supports **three temporal frequencies** configured via `scripts/config.py`:

| Frequency | Time Periods | Total Observations | Build Command |
|-----------|--------------|--------------------|--------------|
| Daily | 4,018 days | 148,666 | `make build-panel-daily` |
| **Weekly** | 575 weeks | 21,275 | `make build-panel-weekly` (default) |
| Monthly | 132 months | 4,884 | `make build-panel-monthly` |

**All downstream scripts respect this configuration automatically.**
...
make build-panel
# Output: data/panel_data_matrix_{frequency}.parquet
#   - Daily: 148,666 obs (4,018 days × 37 stations)
#   - Weekly: 21,275 obs (575 weeks × 37 stations) - DEFAULT
#   - Monthly: 4,884 obs (132 months × 37 stations)
# Frequency controlled by scripts/config.py

# Alternative: Force specific frequency
make build-panel-daily      # Set config to daily and build
make build-panel-weekly     # Set config to weekly and build
make build-panel-monthly    # Set config to monthly and build
...
# Output: data/panel_data_matrix_filtered_for_collinearity_{frequency}.parquet
# Note: File name includes frequency suffix (_daily, _weekly, or _monthly)
```
...
# Note: Temporal analysis adapts to configured frequency
...
[2] build_panel_matrix.py
    ↓ panel_data_matrix_{frequency}.parquet (20 vars)
    ↓ Frequency options: daily (148k obs), weekly (21k obs), monthly (5k obs)
...
[5] filter_multicollinearity.py
    ↓ panel_data_matrix_filtered_for_collinearity_{frequency}.parquet (12 vars)
    ↓
...
1. **Filtered panel data:**
   ...
   - Frequency: Set in `scripts/config.py` (daily/weekly/monthly)
...
**Last Updated:** 10 February 2026  
**Pipeline Version:** v2.1 (with configurable temporal aggregation)
